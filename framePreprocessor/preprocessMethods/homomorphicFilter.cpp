...
cv::Mat createFilter(int rows, int cols, int radius, cv::Size kernel_size)
{
    cv::Mat mask;
    cv::Point center;

    mask = cv::Mat::zeros(rows, cols, CV_32FC1);
    center = cv::Point(cols/2, rows/2);
    cv::circle(mask, center, radius, 1, -1);
    mask = 1 - mask;
    cv::GaussianBlur(mask, mask, kernel_size, 0);
    return mask;
}
...
void homomorphicFilter(const cv::Mat& source, const cv::Mat& dest, float sigma, float alpha, float beta, HighPassFilter& hpf, int borderType) {
    ...
    int isXodd = 0;//(source.cols % 2) == 1;
    int isYodd = 0;//(source.rows % 2) == 1;
    int hpad = 0;//(n - source.cols) / 2;
    int vpad = 0;//(m - source.rows) / 2;
    cv::copyMakeBorder(log, padded, vpad, vpad + isYodd, hpad, hpad + isXodd, borderType);
    //cv::copyMakeBorder(log, padded, 0, 0, 0, 0, borderType);

    // high pass filter
    //cv::Mat highPass = hpf.createFilter(padded.rows, padded.cols, sigma, alpha, beta);
    cv::Size kernel_size = cv::Size(47,47);
    cv::Mat highPass = createFilter(padded.rows, padded.cols, 13, kernel_size);

    //highPass.convertTo(highPass, CV_8UC1, 255.0);
    //print(highPass);
    //cv::imshow("highpass", highPass);
    //cv::imshow("log", log);
    //cv::waitKey();
